function [new_samples, new_time_stamps] = piecewise_geodesic_interpolation_so3(anchor_points, time_stamps, n_total_points)

    time_stamps = time_stamps - time_stamps(1);

    n_anchor_points = size(anchor_points, 3);
    body_velocity = zeros(3, n_anchor_points-1);
    for n = 1:(n_anchor_points-1)
        temp = log_map_so3(anchor_points(:,:,n), anchor_points(:,:,n+1));       
    ...
    new_samples = zeros(3,3,n_total_points);
    ...
    for i = 2:n_total_points
        ...
        new_samples(:,:,i) = exp_map_so3(anchor_points(:,:,previous_anchor_point_index),...
